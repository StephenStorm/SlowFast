...
def pyav_decode(
    ...
    if duration is None:
        ...
        decode_all_video = False
        start_idx, end_idx = get_start_end_idx(
            frames_length,                                  # video size 
            sampling_rate * num_frames / target_fps * fps,  # clip size
            clip_idx,                                       # clip idx
            num_clips,                                      # num_clips
        )
        timebase = duration / frames_length         # time per frames
        video_start_pts = int(start_idx * timebase) # start time 
        video_end_pts = int(end_idx * timebase)     # end time
...
def decode(
    ...
    # print("after pyav decode, before temp")
